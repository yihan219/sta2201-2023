...
model {
  // Log-likelihood
  target += normal_lpdf(log_weight | beta[1]+beta[3]*log_gest+beta[2]*prematurity + beta[4]*combo +beta[5]*log_bmi, sigma);
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_weight_hat_n = beta[1] + beta[3] * log_gest[n] + beta[2]*prematurity[n] + beta[4]*combo[n]+beta[5]*log_bmi[n];
